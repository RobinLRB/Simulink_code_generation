...
// Generated 30-Oct-2020 14:03:46
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"c_caller\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(c_caller_M, (NULL));
        c_caller_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(c_caller_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(c_caller_M)
                == (NULL)) || rtmGetStopRequested(c_caller_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(c_caller_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
