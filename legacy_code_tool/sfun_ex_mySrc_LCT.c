 ...
 *
 *         void myFcn(sigStructType u1[1], paramStructType p1[1], sigStructType y1[1])
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 29-Dec-2020 21:28:59
 *
...
#define S_FUNCTION_NAME  sfun_ex_mySrc_LCT
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "myFcn", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "myFcn_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "sfun_ex_mySrc_LCT_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "sfun_ex_mySrc_LCT_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        DTypeId __sigStructTypeId = ssGetDataTypeId(S, "sigStructType");
        DTypeId __doubleId = ssGetDataTypeId(S, "double");
        DTypeId __paramStructTypeId = ssGetDataTypeId(S, "paramStructType");

        /* Get common data type size */
        __dtSizeInfo[0] = ssGetDataTypeSize(S, __sigStructTypeId);
        __dtSizeInfo[1] = ssGetDataTypeSize(S, __doubleId);
        __dtSizeInfo[2] = ssGetDataTypeSize(S, __paramStructTypeId);

        /* Get information for accessing sigStructType.sig1 */
        __dtBusInfo[0] = ssGetBusElementOffset(S, __sigStructTypeId, 0);
        __dtBusInfo[1] = __dtSizeInfo[1];

        /* Get information for accessing sigStructType.sig2 */
        __dtBusInfo[2] = ssGetBusElementOffset(S, __sigStructTypeId, 1);
        __dtBusInfo[3] = __dtSizeInfo[1];

        /* Get information for accessing paramStructType.param1 */
        __dtBusInfo[4] = ssGetBusElementOffset(S, __paramStructTypeId, 0);
        __dtBusInfo[5] = __dtSizeInfo[1];

        /* Get information for accessing paramStructType.param2 */
        __dtBusInfo[6] = ssGetBusElementOffset(S, __paramStructTypeId, 1);
        __dtBusInfo[7] = __dtSizeInfo[1];

        /* Get information for accessing paramStructType.param3 */
        __dtBusInfo[8] = ssGetBusElementOffset(S, __paramStructTypeId, 2);
        __dtBusInfo[9] = __dtSizeInfo[1];


        /* Alloc memory for the pwork 1 (__y1BUS) */
        {
            sigStructType* __y1BUS = (sigStructType*)calloc(sizeof(sigStructType), ssGetOutputPortWidth(S, 0));
            if (__y1BUS==NULL) {
        ...
        {
            sigStructType* __u1BUS = (sigStructType*)calloc(sizeof(sigStructType), ssGetInputPortWidth(S, 0));
            if (__u1BUS==NULL) {
        ...
        /* Alloc memory for the pwork 3 (__p1BUS) */
        {
            paramStructType* __p1BUS = (paramStructType*)calloc(sizeof(paramStructType), (int_T)mxGetNumberOfElements(ssGetSFcnParam(S, 0)));
            if (__p1BUS==NULL) {
                ssSetErrorStatus(S, "Unexpected error during the memory allocation for __p1BUS");
                return;
            } /* if */
            ssSetPWorkValue(S, 2, __p1BUS);
        }
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        char* p1 = (char*) ssGetRunTimeParamInfo(S, 0)->data;
        char* y1 = (char*) ssGetOutputPortSignal(S, 0);

        sigStructType* __u1BUS = (sigStructType*) ssGetPWorkValue(S, 1);
        paramStructType* __p1BUS = (paramStructType*) ssGetPWorkValue(S, 2);
        sigStructType* __y1BUS = (sigStructType*) ssGetPWorkValue(S, 0);

        /* Assign the Simulink structure u1 to user structure __u1BUS */
        (void) memcpy(&__u1BUS[0].sig1, u1 +  __dtBusInfo[0],  __dtBusInfo[1]);
        (void) memcpy(&__u1BUS[0].sig2, u1 +  __dtBusInfo[2],  __dtBusInfo[3]);

        /* Assign the Simulink structure p1 to user structure __p1BUS */
        (void) memcpy(&__p1BUS[0].param1, p1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__p1BUS[0].param2, p1 +  __dtBusInfo[6],  __dtBusInfo[7]);
        (void) memcpy(&__p1BUS[0].param3, p1 +  __dtBusInfo[8],  __dtBusInfo[9]);

        /* Call the legacy code function */
        myFcn(__u1BUS, __p1BUS, __y1BUS);

        /* Assign the user structure __y1BUS to the Simulink structure y1 */
        (void) memcpy(y1 +  __dtBusInfo[0], &__y1BUS[0].sig1,  __dtBusInfo[1]);
        (void) memcpy(y1 +  __dtBusInfo[2], &__y1BUS[0].sig2,  __dtBusInfo[3]);
    } /* if */
}
...
static void mdlTerminate(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Free memory for the pwork 3 (__p1BUS) */
        {
            paramStructType* __p1BUS = (paramStructType*)ssGetPWorkValue(S, 2);
            if (__p1BUS!=NULL) {
                free(__p1BUS);
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    /* Verify Bus/StructType 'sigStructType', specification */
    {
        uint32_T chk[] = {812450737, 4284203691, 2286625808, 3343240847};
        int_T status;
        status = CheckDataTypeChecksum(S, "sigStructType", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'sigStructType'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'sigStructType' definition has changed since the S-Function was generated");
        }
    }

    /* Verify Bus/StructType 'paramStructType', specification */
    {
        uint32_T chk[] = {1723457635, 2187792853, 1165231518, 3844334818};
        int_T status;
        status = CheckDataTypeChecksum(S, "paramStructType", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'paramStructType'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'paramStructType' definition has changed since the S-Function was generated");
        }
    }
}
